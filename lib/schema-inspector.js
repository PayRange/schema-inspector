...
// Validation ------------------------------------------------------------------
	var _validationAttribut = {
		...
		someKeys: function (schema, candidat) {
			...
			if (!valid) {
				this.errorCustomized = false;
				var _customError = [];
				for(var i = 0; i < _keys.length; i++) {
					var err = schema.properties[_keys[i]].error;
					if(err){
						_customError.push(err);
					}
				}
				if(_customError.length > 0) {
					this.errorCustomized = true;
					this.report(_customError.map(function (i) {
						return i;
					}).join(' or '));
				} else {
			...
			return {
				...
				format: function () {
					...
					return this.error.map(function (i) {
						var keyList = i.property.split('.'),
							obj = schema.properties;
						for(var itr = 0; itr < keyList.length; itr++) {
							if(keyList[itr] !== '@' && obj[keyList[itr]]) {
								obj = obj[keyList[itr]].properties ? obj[keyList[itr]].properties : obj[keyList[itr]];
							}
						}

						if(obj.error || self.errorCustomized) {
							return i.message;
						} else if(i.property != '@') {
							return i.property + ': ' + i.message;
						}
						//Ideally we shouldn't end up with this case.
